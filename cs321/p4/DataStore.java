...
public class DataStore {
	 ...
	 * @throws IOException
	 */
	public DataStore(String filename) throws FileNotFoundException, IOException {
	...
	/**
	 * Constructor: Create a new file. Throws an error if the file already
	 * exists or a read/write error occurs.
	 * 
	 * @param filename the filename
	 ...
	 * @throws IOException
	 */
	public DataStore(String filename, int degree, int subStringLength) throws IOException {
	...
	public void setRoot(int addr) {
		if (addr > endOfFile - recordLength) throw new IndexOutOfBoundsException();
		try {
			dataFile.seek(8);			// Seek to the root address field.
	...
	/**
	 * Write a node's changes to the file.
	 * You need to write changes to a node to disk any time a node is modified.
	 * This includes the first time you give a node any properties.
	 * If you don't, the changes will not persist on the next lookup.
	 * @param node the node
	 */
	public void writeNode(BTreeNode node) {
		...
		try {
			...
			for (int i = 0; i < node.getNumKeys(); i++) {			// Write values to file.
				dataFile.writeInt(node.getValueAt(i));
			}
