...
public class RandomWalk {
	...
	public void step() {
		...
		if (path.contains(next) || next.x < 0 || next.y < 0) {
			...
			if (failures > 3)
				backtrack();
		} else if (next.x >= end.y) {
			next = new Point(end.x, next.y);
			path.add(next);
			failures = 0;
	...
	private Point generateNext() {
		...
		Point next;
		if (randomNum <= .4)
			next = stepEast(path.get(path.size() - 1));
		else if (randomNum > .4 && randomNum <= .8)
			next = stepSouth(path.get(path.size() - 1));
		else if (randomNum > .8 && randomNum <= .9)
			next = stepWest(path.get(path.size() - 1));
		else
			next = stepNorth(path.get(path.size() - 1));
		return next;
	}
