...
public class BTree {
	...
	public BTree(int useCache, int degree, String gbkFilename, int sequenceLength, int cacheSize) {
		...
		try {
			//name of BTree file is "xyz.gbk.btree.data.k.t" - k is sequence length and t is degree
			file = new DataStore(gbkFilename + ".btree.data." + sequenceLength + "." + degree, degree, sequenceLength);
		} catch(IOException e) {
	...
	/**
	 * The secondary constructor that creates a new BTree from a given
	 * BTree file.
	 *
	 * @param useCache whether to use a cache
	 ...
	 * @param cacheSize the cache size (0 if no cache)
	 */
	public BTree(int useCache, DataStore file, int cacheSize){
	...
	public void insert(long newKey) {
		...
		for(int i = 0; i < root.getNumKeys(); i++) {
			if(newKey == root.getKeyAt(i)) {
				root.setValueAt(i, root.getValueAt(i) + 1);
				file.writeNode(root);
				return;
			}
		}
		...
		if(root.getNumKeys() == (2 * degree) - 1) {
			...
			root = currentNode; //need to reassign the root pointer, otherwise it will always point to the same node
		}
		//if not full, insert
		else {
	...
	/**
	 * Recursively inserts a new key into a nonfull node.
	 *
	 * @param currentNode the current, nonfull node
	 * @param newKey the new key
	 */
	public void insertNonfull(BTreeNode currentNode, long newKey) {
		...
		for(int i = 0; i < currentNode.getNumKeys(); i++) {
			if(newKey == currentNode.getKeyAt(i)) {
				currentNode.setValueAt(i, currentNode.getValueAt(i) + 1);
				file.writeNode(currentNode);
		...
		//if not a leaf node, need to find one to insert into
			...
			if(child1.getNumKeys() == (2 * degree) - 1) {
				...
				//if the new key is greater than the value brought up into the current node when splitting the child node
				if(newKey > currentNode.getKeyAt(i)) {
					i++;
				}
			}
	...
	public void splitChild(BTreeNode currentNode, int i) {
		...
		for(int j = 0; j < degree - 1; j++) {
			...
			child2.setValueAt(j, child1.getValueAt(j + degree));
		}
		...
		//if currentNode is empty and only points to child1, the for loop will not be performed
		...
		for(int j = currentNode.getNumKeys() - 1; j >= i; j--) {
			...
			currentNode.setValueAt(j + 1, currentNode.getValueAt(j));
		}

		currentNode.setKeyAt(i, child1.getKeyAt(degree - 1));
		currentNode.setValueAt(i, child1.getValueAt(degree - 1));
		currentNode.setNumKeys(currentNode.getNumKeys() + 1);
	...
	public int search(BTreeNode currentNode, long searchKey) {
		...
		//if the searched-for key matches the key at the index, return the number of instances at that index
		if(i < currentNode.getNumKeys() && searchKey == currentNode.getKeyAt(i)) {
			return currentNode.getValueAt(i);
		}
		//if current node is a leaf node and the value is not found, end search and return 0
	...
	/**
	 * Gets the DataStore file.
	 * @return the DataStore file
	 */
	public DataStore getDataFile(){
        return file;
    }
