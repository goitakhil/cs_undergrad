...
/**
 * Cache implementation. Checks cache for a node.
 * If not found, it will query the DataStore for the node.
 * @author Tyler Bevan
 */
public class Cache {
	private int size;
	 ...
	 */
	public Cache(int size, DataStore store) {
		this.size = size;
		this.store = store;
		this.list = new LinkedList<BTreeNode>();
	}

	/** cacheQuery checks the cache for a node by address.
	 * If not found, it will query the DataStore for the node 
	 * and add it to the cache. It will trim the LinkedList to keep 
	 * its size from growing beyond the given bound.
	...
	/** Clears the cache by creating a new
	 * LinkedList.
	 */
